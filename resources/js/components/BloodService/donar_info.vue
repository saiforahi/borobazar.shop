        ...
        <div class="bloodreg-inner">
            <form class="bloodreg-form row">
                <div class="col-lg-6 form-group">
                    ...
                    <input type="text" class="form-control2" v-model="firstName"  id="firstName" disabled>
                </div>
                <div class="col-lg-6 form-group">
                    <label for="text">নামের শেষ অংশ</label>
                    <input type="text" class="form-control2" v-model="lastName"  id="lastName" disabled>
                </div>
                <div class="col-lg-6 form-group">
                ...
                <div class="form-group col-lg-6" >
                    ...
                    <select :disabled=" registered===true " class="form-control2 search-blood" id="bloodGroup" name="bloodGroup" v-model="selectedBloodGroup">
                        <option v-if="registered==false"  disabled value="-1">রক্তের গ্রুপ</option>
                        <option v-for="bloodGroup in bloodGroups" :value="bloodGroup.id" v-bind:key="bloodGroup.id">{{bloodGroup.bangla}}</option>
                    </select>
                    <p v-if="errors.bloodGroup==true"><code>সঠিক গ্রুপ দিন</code></p>
                </div>
                <div class="col-lg-6 form-group">
                ...
                <button v-if="registered==true" @click="save" type="button" class="btn-getinvite" id="submit">আপডেট করুন</button>
                <button v-else-if="registered==false" @click="save" type="button" class="btn-getinvite" id="submit">নিবন্ধন করুন</button>
            </form>
        </div>
    </div>
</template>
...
export default {
    ...
    mounted(){
        axios.get('/api/user/show_donar_info').then(response => {
            ...
            if(response.data.registered==false){
                ...
                if(response.data.donar_info.hasCell==true){
                    this.userHasCell=true;
                    this.cell=response.data.donar_info.cell
                }
                else{
    ...
    methods:{
        ...
        save(){
            if(this.selectedBloodGroup>0 && this.cell!=undefined){
            ...
            else if(this.cell==undefined && this.selectedBloodGroup>0){
                this.errors.cell=true;
            }
            else if(this.cell!=undefined && this.selectedBloodGroup<=0){
                this.errors.bloodGroup=true;
